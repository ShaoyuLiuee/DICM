...
#         one_hot_label = torch.nn.functional.one_hot(torch.tensor(label), num_classes=10).float()
#         one_hot_label[one_hot_label == 0] = 0.49
#         one_hot_label[one_hot_label == 1] = 0.51        
...
#         one_hot_label = one_hot_label.view(1, 10, 1, 1)  
#         one_hot_label = one_hot_label.expand(-1, -1, 96, 96) 

#         processed_image = torch.cat((image_tensor, one_hot_label), dim=1) 
...
#         processed_image = self.preprocess_image(image, label)
#         return processed_image
...
class ImageNet:
    ...
    def __init__(self, root, split="train", transform=None):
        ...
        self.data_folder = tvds.ImageFolder(os.path.join(root, 'train' if split == "train" else 'val'), transform=self.transform)

    def preprocess_image(self, image, label):
        one_hot_label = torch.nn.functional.one_hot(torch.tensor(label), num_classes=200).float()
        ...
        one_hot_label = one_hot_label.view(1, 200, 1, 1)
        one_hot_label = one_hot_label.expand(-1, -1, 64, 64)
    ...
    def __getitem__(self, index):
        image, label = self.data_folder[index]
        processed_image = self.preprocess_image(image, label)
        return processed_image
...
#         one_hot_label = one_hot_label.view(1, 10, 1, 1)  # 调整维度为（1 × 10 × 1 × 1）
#         one_hot_label = one_hot_label.expand(-1, -1, 32, 32)  # 扩展维度以匹配图像的高度和宽度

#         processed_image = torch.cat((image_tensor, one_hot_label), dim=1) #在这个维度上，张量的大小必须是一致的，除了拼接的维度之外，其他维度的大小必须相同。
