...
class Trainer:
    ...
    def loss(self, x, current_epochs):
        loss = self.diffusion.train_losses(self.model, **self.get_input(x), current_epochs=current_epochs)
        assert loss.shape == (x.shape[0],)
        return loss

    def step(self, x, global_steps=1, current_epochs=1):
        ...
        loss = self.loss(x, current_epochs).mean()
        ...
        for e in range(self.start_epoch, self.epochs):
            ...
            with tqdm(self.trainloader, desc=f"{e + 1}/{self.epochs} epochs", disable=not self.is_leader) as t:
                for i, x in enumerate(t):
                    ...
                    global_steps += 1
                    self.step(x.to(self.device), global_steps=global_steps, current_epochs=e)
