...
class ManifoldBuilder:
    def __init__(
        ...
        features = features.to(torch.float32)  # half precision for faster distance computation?
    ...
    def compute_kth(self, row_features: torch.Tensor, col_features: torch.Tensor = None):
        ...
        for row_batch in tqdm(row_features.split(self.row_batch_size, dim=0), desc="Computing k-th radii"):
            ...
            kth.append(dist_batch.to(torch.float32).kthvalue(self.nhood_size + 1, dim=1).values.to(torch.float32))  # nhood_size + 1 to exclude itself
        kth = torch.cat(kth)
        return kth
