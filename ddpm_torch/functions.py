...
def normal_kl(mean1, logvar1, mean2, logvar2):
    ...
    kl = (-1.0 - diff_logvar).add(
        (mean1 - mean2).pow(2) * torch.exp(-logvar2)).add( #torch中的exp函数是指数函数,用于将输入的每个元素取e的指数幂。
        torch.exp(diff_logvar)).mul(0.5) #点对点相乘
    return kl
...
def discrete_klv2d(hist1, hist2, eps=1e-9):
    """
    compute the discretized (empirical) Kullback-Leibler divergence between P_data1 and P_data2,计算P_data1和P_data2之间离散化的(经验的)Kullback-Leibler散度
    """
...
def hist2d(data, bins, value_range=None):
    ...
    if value_range is not None:
        ...
        if hasattr(value_range, "__iter__"): #判断object对象中是否存在name属性
            if not hasattr(next(iter(value_range)), "__iter__"):
                value_range = (value_range, ) * 2
    x, y = np.split(data, 2, axis=1)
    x, y = x.squeeze(1), y.squeeze(1)
    return np.histogram2d(x, y, bins=bins, range=value_range)[0] #用于生成2D直方图


def flat_mean(x, start_dim=1):
    reduce_dim = [i for i in range(start_dim, x.ndim)]
    return torch.mean(x, dim=reduce_dim) #在什么维度上求平均


def flat_sum(x, start_dim=1): #只是定义了  但是没用
    reduce_dim = [i for i in range(start_dim, x.ndim)]
