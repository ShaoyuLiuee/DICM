...
class UNet(nn.Module):
    ...
    def __init__(
        ...
        self.linear_mapping = nn.Linear(10, 128)
        ...
        self.in_conv = Conv2d(3, hid_channels, 3, 1, 1)
        # # level1
        self.in_conv_label = Conv2d(10, hid_channels, 3, 1, 1)
        # # level2
        # self.in_conv_label = nn.Conv2d(in_channels=100, out_channels=256, kernel_size=3, stride=2, padding=1)
        # # level3
        # self.in_conv_label = nn.Conv2d(in_channels=100, out_channels=256, kernel_size=4, stride=4, padding=0)
        self.levels = levels
        ...
        self.out_conv = Sequential(
            ...
            self.nonlinearity,
            Conv2d(hid_channels, 13, 3, 1, 1, init_scale=0.)
        )
        # # # level1
        # self.out_conv_label = Sequential(
        #      self.normalize(hid_channels),
        #      self.nonlinearity,
        #      Conv2d(hid_channels, 10, 3, 1, 1, init_scale=0.)
        # )
        # # level2
        # self.out_conv_label = Sequential(
        #     self.normalize(256),
        #     self.nonlinearity,
        #     ConvTranspose2d(256, 100, kernel_size=4, stride=2, padding=1, output_padding=0)
        # )        
        # # level3
        # self.out_conv_label = Sequential(
        #    self.normalize(256),
        #    self.nonlinearity,
        #    ConvTranspose2d(in_channels=256, out_channels=100, kernel_size=4, stride=4, padding=0)
        # )   
    ...
    def _get_downsample_by_level(self, level):
        block = self._get_block_by_level(level) # 根据给定的层级返回相应的block
        prev_chans = (self.ch_multipliers[level-1] if level else 1) * self.hid_channels # 计算前一层通道数
        curr_chans = self.ch_multipliers[level] * self.hid_channels # 计算当前通道数
        modules = nn.ModuleList([block(prev_chans, curr_chans)]) # 创建一个包含一个基础块的模型列表
        for _ in range(self.num_res_blocks - 1):
            modules.append(block(curr_chans, curr_chans))
        if level != self.levels - 1: # 如果不是最后一层 执行下述语句
            if self.resample_with_conv: # 检查是否使用卷积下采样
    ...
    def forward(self, x, t):
        ...
        x_image = self.in_conv(x_image)
        x_label = self.in_conv_label(x_label)
        x_in_conv = x_image + x_label
        hs = [x_in_conv]    
